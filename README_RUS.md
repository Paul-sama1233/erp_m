...
## Contents
  [1.1. Общая инструкция](#11-общая-инструкция)
- [Глава 2](#глава-2) \
  [2.1. Введение](#21-введение)
- [Глава 3](#глава-3) \
  [3.1. Про мобильное тестирование](#31-про-мобильное-тестирование) \
  [3.2. Appium](#32-appium) \
  [3.3. Appium Inspector](#33-appium-inspector) \
  [3.4. Espresso](#34-espresso)
- [Глава 4](#глава-4) \
  [4.1. Начало работы с фреймворком](#41-начало-работы-с-фреймворком) \
  [Задание 1. Работа с Espresso](#задание-1-работа-с-espresso)
...
В проекте QA08_Mobile testing (программа «Ручное тестирование») мы уже говорили о тонкостях мобильного тестирования и о том, что мобильные приложения — это не просто «маленькие версии» веб-сайтов. 
...
| :- | :- | :- |
| - Проверка авторизации; <br>- Работа с геолокацией, камерой, уведомлениями; <br>- Тесты на жесты (свайп, долгое нажатие). | - Переключение между нативным контекстом и WebView; <br>- Тестирование встроенных веб-страниц. | - Взаимодействие с другими приложениями (например, переход из мессенджера в браузер). |
...
1. Открой Appium Inspector (в составе Appium Desktop).
1. Укажи параметры сессии (capabilities): платформа, версия ОС, путь к приложению.
1. Подключись к устройству/эмулятору.
...
| :- | :- |
| - Визуальное выделение элементов на скриншоте; <br>- Поддержка локаторов: ID, XPath, className; <br>- Генерация шаблонного кода на Java/Python. | - Для iOS требуется настройка Xcode и подписанное приложение; <br>- Сложности с гибридными приложениями (WebView); <br>- Иногда нестабилен при работе с динамическими элементами. |
...
1) Реализуй следующие тесты в следующих классах:
   ...
   1) IntentTest — проверить, что мы в первой активности, открыть вторую активность, проверить, что мы в ней и текст на экране соответствует необходимому.
   1) TransitionTest — проверить переходы между всеми экранами приложения (часть переходов будет осуществляться использованием системной кнопки «назад»).
   1) ToastTest — проверить текст, всплывающий при нажатии кнопки с письмом в правом нижнем углу экрана.
   1) ToolbarTest — проверить, что при нажатии на три точки в правом верхнем углу экрана появляется выпадающее меню с опцией «Settings», а при нажатии в другую часть экрана оно исчезает.
...
![package](misc/images/package.png)
