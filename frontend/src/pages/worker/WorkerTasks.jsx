...
export default function WorkerTasks() {
  ...
  return (
    <div style={s.page}>
      <h2 style={s.title}>Мои изделия</h2>
        ...
        <div style={s.overlay}>
          <div style={s.modal}>
            ...
            <p style={{ color: '#888', marginBottom: 16, fontSize: 14 }}>
              Изделие: <strong>{writeoffForm.product_name}</strong>
            </p>
            {writeoffForm.available_materials.length === 0 ? (
              <p style={{ color: '#aaa', marginBottom: 16 }}>Нет материалов для списания</p>
            ) : (
                  ...
                  <div key={m.id} style={sw.row}>
                    <div style={sw.matInfo}>
                      ...
                      <div style={sw.matStock}>
                        Склад: {m.quantity} {m.unit} | Норма: {m.needed}
                      </div>
                    </div>
                    <input style={sw.input} type="number" step="0.01" min="0"
      ...
      {active.length > 0 && (
        <>
          <h3 style={s.sectionTitle}>Активные задачи</h3>
          <div style={s.grid}>
            {active.map(stage => (
              <div key={stage.id} style={s.card}>
                ...
                <div style={s.stageName}>
                  {STAGE_LABELS[stage.stage_type]} • Этап #{stage.order + 1}
                </div>
      ...
      {done.length > 0 && (
        <>
          <h3 style={{ ...s.sectionTitle, marginTop: 32 }}>Завершённые</h3>
          <div style={s.grid}>
            {done.map(stage => (
              <div key={stage.id} style={{ ...s.card, opacity: 0.65 }}>
                ...
                <div style={s.stageName}>{STAGE_LABELS[stage.stage_type]}</div>
                {stage.completed_at && (
                  <div style={s.date}>
